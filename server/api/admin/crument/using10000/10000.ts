...
export default defineEventHandler(async (event) => {
  ...
  if (method === "PUT") {
    try {
      ...
      const using10000Id = parseInt(
        formData.find((item) => item.name === "id")?.data.toString() || "0"
      );
      const existingUsing = await prisma.using10000.findUnique({
      ...
      if (imageFile && imageFile.filename) {
        ...
        const uniqueFilename = `using10000/${Date.now()}-${Math.random()
          .toString(36)
          .substring(2)}.${fileExt}`;
      ...
      const updatedUsing = await prisma.using10000.update({
        where: { id: using10000Id },
        data: {
...
//api/admin/crument/using10000/10000.ts
